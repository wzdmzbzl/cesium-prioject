...
// Cesium.Matrix4.multiplyByPoint(matrix, cartesian, result)：将一个点与矩阵相乘。
// matrix：要进行乘法计算的矩阵对象。
// cartesian：要乘以矩阵的点的笛卡尔坐标对象。
// result：可选参数，输出结果的笛卡尔坐标对象。
// Cesium.Matrix4.multiplyByVector(matrix, vector, result)：将一个向量与矩阵相乘。
// matrix：要进行乘法计算的矩阵对象。
// vector：要乘以矩阵的向量对象。
// result：可选参数，输出结果的向量对象。
// Cesium.Matrix4.computePerspectiveFieldOfView(fovY, aspectRatio, near, far, result)：计算透视投影矩阵。
// fovY：视场角，以弧度表示。
// aspectRatio：视口宽高比。
// near：近平面距离。
// far：远平面距离。
// result：可选参数，输出结果的矩阵对象。
// Cesium.Matrix4.computeOrthographicOffCenter(left, right, bottom, top, near, far, result)：计算正交投影矩阵。
// left：左平面距离。
// right：右平面距离。
// bottom：底部平面距离。
// top：顶部平面距离。
